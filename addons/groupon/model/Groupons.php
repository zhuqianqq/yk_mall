...
class Groupons extends Base{
	...
	public function submitByEntity($carts,$orderSrc = 0,$uId=0){
		...
		try{
			...
			if($deliverType==1){//自提
				...
				$order['storeType'] = 1;
				$order['userName'] = input("store_userName");
				$order['userPhone'] = input("store_userPhone");
			}
	...
	*/
	public function illegal(){
		...
		$rs = $this->alias('gu')
		           ...
		           ->where('grouponId',$id)
		           ->field('gu.goodsName,gu.grouponId,g.shopId,s.userId,g.goodsSn,gu.grouponStatus,g.goodsId')->find();
    ...
    */
    public function createPoster($userId,$qr_code,$outImg){
        ...
        $id = input("id");
        $goods = Db::name("goods g")->join("groupons gp","g.goodsId=gp.goodsId","inner")->where(['gp.grouponId'=>$id])->field("g.goodsId,g.goodsName,g.goodsImg,gp.grouponPrice shopPrice,g.goodsUnit")->find();
        ...
        $text2 = mb_convert_encoding('团购价：', "html-entities", "utf-8"); //转成html编码
