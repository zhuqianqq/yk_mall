...
function changeAddrId(id){
	$.post(WST.U('home/useraddress/getById'),{id:id},function(data){
		...
		if(json.status==1){
			...
			$("#store_userName").val(json.data.userName);
			$("#store_userPhone").val(json.data.userPhone);
...
function checkSupportStores(){
	...
	$.post(WST.AU('groupon://carts/checkSupportStores'),{addressId:addressId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data){
    			$("#deliver_btn").show();
...
function checkStores(shopId){
	...
	$('#st_userName').val($("#store_userName").val());
 	$('#st_userPhone').val($("#store_userPhone").val());
	...
	layerbox =	layer.open({
				    ...
				    yes:function(){
						...
						$("#store_userName").val(st_userName);
					 	$("#store_userPhone").val(st_userPhone);
