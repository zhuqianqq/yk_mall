	...
	$('#tab').TabPanel({tab:0,callback:function(no){
		...
		if(!initBatchUpload && no==2){
			...
			var uploader = batchUpload({uploadPicker:'#batchUpload',uploadServer:WST.U('shop/index/uploadPic'),formData:{dir:'goods',isWatermark:1,isThumb:1},uploadSuccess:function(file,response){
				...
				if(json.status==1){
					$li = $('#'+file.id);
					$li.append('<input type="hidden" class="j-gallery-img" iv="'+json.url +'" v="' +json.url+'"/>');
	...
	WST.upload({
	  	  ...
	  	  callback:function(f){
	  		  ...
	  		  if(json.status==1){
	  			  ...
	  			  $('#goodsVideoPlayer').attr('src',json.url);
	              $('#goodsVideo').val(json.url);
...
function addSpecImg(opts){
	...
	html.push('<tr>',
	            ...
	            (opts.specImg)?'<img height="25"  width="25" id="specImg_'+opts.catId+'_'+specNum+'" src="'+opts.specImg+'" v="'+opts.specImg+'"/><span class="item-del" onclick="clearSpecImg(this,'+specNum+')"></span>':"",
	...
	WST.upload({
		  ...
		  cat:opts.catId,
	  	  ...
	  	  callback:function(f){
	  		  var json = WST.toJson(f);
	  		  if(json.status==1){
	  			$('#uploadMsg_'+this.cat+"_"+this.num).html('<img id="specImg_'+this.cat+"_"+this.num+'" v="'+json.url+'" src="'+json.url+'" height="25"  width="25"/><span class="item-del" onclick="clearSpecImg(this,'+this.num+')"></span>');
	  		  }
		  },
		  progress:function(rate){
...
function saleByPage(p){
    ...
    var cols = [
        {title:'商品图片', name:'goodsName', width: 80, renderer: function(val,item,rowIndex){
        	    var goodsImg = WST.buildImgUrl(item['goodsImg']);
                return "<span class='weixin'><a style='color:blue' href='javascript:toDetail("+ item['goodsId']+",\""+item['verfiycode']+"\")'><img class='img' style='height:50px;width:50px;' src='" + goodsImg
					+"'><img class='imged' style='height:200px;width:200px;border:0px; background:#fff' src='" + goodsImg + "'></a></span>";
            }},
...
function auditByPage(p){
    ...
    var cols = [
        {title:'商品图片', name:'goodsName', width: 80, renderer: function(val,item,rowIndex){
				var goodsImg = WST.buildImgUrl(item['goodsImg']);
                return "<span class='weixin'><a style='color:blue' href='javascript:toDetail("+ item['goodsId']+",\""+item['verfiycode']+"\")'><img class='img' style='height:50px;width:50px;' src='" + goodsImg
					+"'><img class='imged' style='height:200px;width:200px;' src='" + goodsImg + "'></a></span>";
            }},
        {title:'商品名称', name:'goodsName', width: 250, renderer: function(val,item,rowIndex){
        	return "<a style='color:#666' href='javascript:toDetail("+ item['goodsId']+",\""+item['verfiycode']+"\")'>"+val+"</a> ";
...
function illegalByPage(p){
    ...
    var cols = [
        {title:'商品图片', name:'goodsName', width: 80, renderer: function(val,item,rowIndex){
                return "<span class='weixin'><a style='color:blue' href='javascript:toDetail("+ item['goodsId']+",\""+item['verfiycode']+"\")'><img class='img' style='height:50px;width:50px;' src='"+item['goodsImg']+"'><img class='imged' style='height:200px;width:200px;' src='"+item['goodsImg']+"'></a></span>";
            }},
        {title:'商品名称', name:'goodsName', width: 250, renderer: function(val,item,rowIndex){
        	return "<a style='color:#666' href='javascript:toDetail("+ item['goodsId']+",\""+item['verfiycode']+"\")'>"+val+"</a> ";
