...
class Weixinpays extends Base{
	...
	public function toWeixinPay(){
		...
		$needPay = 0.01;
		//实例化微信预支付库控制器
		$WxPrePay =new \util\WxPrePayLib($this->wxpayConfig);

		$total_fee = $needPay * 100;//支付金额
		
		$notify_url = $this->wxpayConfig ['notifyurl'];
		
		$order = $WxPrePay->getPrePayOrder($body, $out_trade_no, $total_fee, $notify_url);

		if (isset($order['prepay_id'])){//判断返回参数中是否有prepay_id
			$second_sign = $WxPrePay->getOrder($order['prepay_id']);
			$prepay_json = json_encode($second_sign);
			$this->assign('prepay_json',$prepay_json);
			return $this->fetch('users/orders/orders_app_wxpay');
		} else {
			return '支付失败！';
		}
	...
	public function notify() {
        ...
        Tools::addLog("wx_app_notify", "微信APP回调开始,praram:$xml");
