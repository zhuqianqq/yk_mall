...
function areaShow() {
  ...
  jQuery('#address-frame').animate({ 'bottom': 0 }, 500)
}

function dataHide() {
  var dataHeight = $('#address-frame').css('height')
  jQuery('#address-frame').animate({ 'bottom': '-' + dataHeight }, 500)
  jQuery('#cover').hide()
  scrollFlag = 0
  setTimeout(function () {
    jQuery('#address-frame').hide()
  }, 500)
}
...
function saveAddress() {
  ...
  var param = {}
  var addressId = $('#addressId').val() || 0
  param.addressId = addressId
  param.userName = userName
  param.areaId = areaId
  param.userPhone = userPhone
  param.userAddress = userAddress
  param.isDefault = 1 // 设为默认地址
  $.post(WST.U('mobile/useraddress/edits'), param, function (data) {
    ...
    if (json.status == 1) {
      ...
      $('#addressId').val(addressId) // 收货地址保存成功后提交订单
      submitOrder()
    } else {
  ...
  $.post(WST.U('mobile/useraddress/setDefault'), { id: id }, function (data) {
    ...
    if (json.status == 1) {
      ...
      setTimeout(function () {
        ...
        location.replace(WST.U('mobile/useraddress/index'))
      }, 1500)
    } else {
      WST.msg(json.msg, 'warn')
