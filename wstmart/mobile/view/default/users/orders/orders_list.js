...
function getOrderList() {
  ...
  $.post(WST.U('mobile/orders/getOrderList'), param, function (data) {
    ...
    if (json && json.data && json.data.length > 0) {
      ...
      html += '<div class="hj-empty"><img src="https://img.ikstatic.cn/MTU4NDA3MDc5NDYyNSM3MjAjcG5n.png"></div>'
      html += '<div class="wst-prompt-info hj-wst-prompt-info">'
      html += '<p>您还没有相关订单</p>'
...
function countDown() {
  ...
  function countDownOnce() {
    $('.time_end').each(function (index, item) {
      var $current = $(item)
      var creatTime = $current.data('create-time').replace(/-/g, '/') // 兼容ios
      var timeStamp = new Date(creatTime).getTime()
...
function getOrderDetail(oid,pkey){
  location.href = '/mobile/orders/getOrderDetail?id='+ oid+ '&pkey=' + pkey;
  ...
  setTimeout(function () {
    $.post(WST.U('mobile/wallets/payByWallet'), params, function (data, textStatus) {
      ...
      if (json.status == 1) {
        ...
        location.href = WST.U('mobile/orders/finishpay', { 'orderNo': data.data.orderId })
