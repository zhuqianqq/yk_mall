...
//                $order['useScore'] = $shopOrderMoney['useScore'];
	...
	public function userOrdersByPage($orderStatus, $isAppraise = -1, $uId = 0)
    {
        ...
        $formartOrder = array_column($page['data'],null, 'orderId');
	    if (count($page['data']) > 0) {
	    	 ...
	    	 foreach ($goods as $v) {
                ...
                if ($v['goodsSpecNames'] != "") {
                 ...
                 $orderRefundStatus[$v['orderId']] = 1;
                if (!in_array($v['refundStatus'], $refundingStatus)) {
                    $orderRefundStatus[$v['orderId']] = 0;
                }
	    	 ...
	    	 foreach ($page['data'] as $key => $v) {
                 ...
                 if ($v['isRefund'] == 1) {//有退款的情况
                    if ($orderRefundStatus[$v['orderId']] == 1) { //所有商品都再退款中
                        $page['data'][$key]['orderStatusName'] = WSTLangOrderListStatus(-3);//退款中
                        $page['data'][$key]['orderStatus'] = -3;
                    } /*else {
                        if (in_array($v['refundStatus'],[Refund::REFUND_CANCEL])) {
                            $page['data'][$key]['orderStatusName'] = WSTLangOrderListStatus($v['orderStatus']);
                        } else
                            $page['data'][$key]['orderStatusName'] = WSTLangRefundStatus($v['refundStatus']);
                    }*/
                     $page['data'][$key]['orderStatusName'] = WSTLangOrderListStatus($v['orderStatus']);
                 }else
                     $page['data'][$key]['orderStatusName'] = WSTLangOrderListStatus($v['orderStatus']);
...
//	    	 hook('afterQueryUserOrders',['page'=>&$page]);
	...
	public function shopOrdersByPage($orderStatus, $sId=0){
	    ...
	    if(count($page['data'])>0){
	    	 ...
	    	 foreach ($page['data'] as $key => $v){
	    	 	 ...
	    	 	 $page['data'][$key]['status'] = WSTLangOrderStatusSeller($v['orderStatus']);
	    	 }
	    }
	    return $page;
	}
     ...
     * 商家修改物流，只可修改一次
     */
    public function updateExpress($uId=0, $sId=0){
        $orderId = (int)input('post.id');
        $redisKey = "SHOP:UPDATE:EXPRESS:ORDERID:" . $orderId;
        $isUpdate = Cache::get($redisKey);
        if ($isUpdate) {
            return WSTReturn('不可修改');
        }
        $expressId = (int)input('post.expressId');
        $expressNo = ($expressId>0)?input('post.expressNo'):'';
        $shopId = ($sId==0)?(int)session('WST_USER.shopId'):$sId;
        ...
        $deliverType = (int)input('post.deliverType');
        // 查询已发货的订单
        $order = $this->where(['shopId'=>$shopId,'orderId'=>$orderId,'orderStatus'=>1])->field('orderId,orderNo,userId,deliverType')->find();
        if(empty($order))return WSTReturn('操作失败，请检查订单状态是否已改变');
        // 前台传过来的deliverType,deliverType==1需要物流，deliverType=0无需物流
        $deliverType = ($deliverType==1)?1:0;
        if ($deliverType == 1) {
        ...
        //验证前台传过来的数据
        //首先去order_express表查询已发货的orderGoodsId
        $orderExpressGoodsIds = Db::name('order_express')->field('orderGoodsId')->where('orderId','=',$orderId)->find();
        if(empty($orderExpressGoodsIds)){
            return WSTReturn('暂时没有物流信息');
        }
        Db::startTrans();
        try{
            //只要需要商家发货的都插入订单物流表里
            ...
            Db::name('order_express')->where("orderId = {$orderId}")->update($expressData);
            $orderStatus = 1;
            //新增订单日志
            $logOrder = [];
            $logOrder['orderId'] = $orderId;
            $logOrder['orderStatus'] = $orderStatus;
            $logOrder['logContent'] = "商家修改物流信息";
            $logOrder['logUserId'] = $userId;
            $logOrder['logType'] = 0;
            $logOrder['logTime'] = date('Y-m-d H:i:s');
            Db::name('log_orders')->insert($logOrder);
	...
	/**
	 * 未付款状态（-2） 用户可手动取消订单  注意加库存操作 
	 * 
	 */
	public function quxiaoOrderH5($uId=0){
		$orderId = (int)input('post.id');
		$userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
		$order = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		->where([['o.orderStatus','=',-2],['o.userId','=',$userId],['o.orderId','=',$orderId]])
		...
		if(!empty($order)){
			Db::startTrans();
		    try{
                $data = ['orderStatus'=> 6 ,'cancelReason'=>'用户手动取消订单'];
				//把实付金额设置为0
				...
				if(false != $result){
					...
					$logOrder['logContent'] = "用户取消订单，取消原因：手动取消订单";
					$logOrder['logUserId'] = $userId;
					$logOrder['logType'] = 0;
					$logOrder['logTime'] = date('Y-m-d H:i:s');
					Db::name('log_orders')->insert($logOrder);
	...
	*/
     ...
     */
    public  function getSellerOrders($shop_id)
    {
        $time = date('Y-m-d 00:00:00');
        $isPay = 1;
        $orders = $this->where(" shopId = {$shop_id}  and payTime >= '{$time}' and isPay = {$isPay} ")->field('sum(realTotalMoney) as totalMoney, count(*) totalCount')->find();
        return $orders;
    }
