...
class OrderRefunds extends Base{
    ...
    public function refundPageQuery($userId = 0)
    {
        ...
        $list = Db::name('order_goods')->alias('og')
            ...
            ->field('og.orderId,og.goodsSpecId,og.goodsId,og.goodsNum,og.goodsPrice,og.goodsSpecNames, og.goodsName, og.goodsImg, orf.refundStatus, orf.createTime, orf.backMoney')
            ->order('orf.createTime', 'desc')
            ->paginate(input('limit/d'))->toArray();
        if (!empty($list['data'])) {
            foreach ($list['data'] as $k => $v) {
                $list['data'][$k]['refundMoney'] = $v['backMoney'];
...
//            }
    ...
    public function orderDetailrefund()
    {
		...
		->field('og.orderId,og.goodsId,og.goodsNum,og.goodsPrice,og.goodsSpecId,og.goodsSpecNames, og.goodsName, og.goodsImg,orf.id as refundId,orf.refundTo,
		 orf.refundStatus,orf.refundTradeNo,orf.refundReson,orf.refundOtherReson,orf.backMoney,orf.refundRemark,orf.logisticTime,orf.logisticNum,orf.logisticInfo,orf.createTime,orf.refundTime,orf.shopAgreeTime,o.orderStatus,o.shopId,o.createTime as oCreateTime,o.payTime,o.receiveTime,o.deliveryTime')
		->order('orf.createTime', 'desc')
		->find() ?? [];
