...
class Carts extends Base{
	 ...
	 * 验证商品是否合法
	 */
	public function checkGoodsSaleSpec($goodsId,$goodsSpecId, $uid = 0){
		$goods = model('Goods')->where(['goodsStatus'=>1,'dataFlag'=>1,'isSale'=>1,'goodsId'=>$goodsId])->field('goodsId,isSpec,goodsStock,goodsType,shopId')->find();
		...
		if ($shop['userId'] == $uid) {
            return WSTReturn("添加失败，不能购买自己的商品", -1);
        }
		//有规格的话查询规格是否正确
		if($goods['isSpec']==1){
