...
class CashDraws extends Base{
      ...
      */
      ...
      public function drawMoney($uId=0){
          ...
          $actualMoney = 0;
          $commission = 0;
          $cashCommission = (float)WSTConf('CONF.drawCashCommission');
          if($cashCommission>0){
              $commission = $money*$cashCommission*0.01;
     ...
     * @throws \Exception
     */
    public function drawMoneyByShopByUserid($user_id) {
        if (empty($user_id)) {
            throw new \Exception("缺少参数");
        }
        $shops = model('shops')->where("userId = {$user_id}")->find();
        if (empty($shops)) {
            throw new \Exception("没有数据");
        }
        $money = $shops->shopMoney;

        $limitMoney = (float)WSTConf('CONF.drawCashShopLimit');
        if($money < $limitMoney) throw new \Exception('提取金额必须大于或等于￥'.$limitMoney.'方可提现');
        ...
        $alipayAccount = model('user_alipay_account')->where("user_id = {$userid}")->find();
        ...
        try{
            ...
            if(false !==$result){
                ...
                $data['commission'] = $commission;
                $data['actualMoney'] = $actualMoney;
                $data['alipayNo'] = $alipayAccount['account_num'];
                $data['alipayUserName'] = $alipayAccount['true_name'];
                $this->save($data);
                $this->cashNo = $this->cashId.(fmod($this->cashId,7));
                $this->save();
                Db::commit();
