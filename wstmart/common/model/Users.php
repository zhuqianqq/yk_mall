...
class Users extends Base
{
    ...
    public function checkLogin($loginSrc = 0)
    {
        ...
        if (!empty($rs)) {
            if ($rs['loginPwd'] != md5($loginPwd . $rs['loginSecret'])) {
                return WSTReturn("密码错误");
            }
            if ($rs['userPhoto'] == '') {
                $rs['userPhoto'] = WSTConf('CONF.userLogo');
            }
            $userId = $rs['userId'];
            //获取用户等级
            $rrs = Db::name('user_ranks')->where(['dataFlag' => 1])
                     ...
                     ->field('rankId,rankName,userrankImg')->find();
    ...
    {
        $rs = $this->where(["userId" => $user_id, "userStatus" => 1])->find();
...
//            }
			...
			//Tools::addLog("WST_USER", json_encode(session('WST_USER')));
...
//            return WSTReturn("短信验证码不一致，请重新输入！", -1);
        ...
        if (!empty($rs)) {
            ...
            if (WSTConf('CONF.signScoreSwitch') == 1) {
                ...
                $userscores = Db::name('user_scores')->where([["userId", '=', $userId], ["dataSrc", '=', 5]])->where('left(createTime,7)="' . date('Y-m') . '"')->order('createTime desc')->find();
                if ($userscores) {
                    $rs['signScoreTime'] = date("Y-m-d", strtotime($userscores['createTime']));
                }
            }
            ...
            SmsHelper::clearCacheKey($loginName);
