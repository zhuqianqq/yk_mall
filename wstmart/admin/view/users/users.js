...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'昵称', name:'userName' ,width:100,sortable:true,renderer:function(val,item,rowIndex){
                var html = '';
                if(item['wxOpenId'] && item['wxOpenId']!='')html = html+'<img class="order-source2" src="'+WST.conf.ROOT+'/wstmart/admin/view/img/order_source_1.png"> ';
                if(item['weOpenId'] && item['weOpenId']!='')html = html+'<img class="order-source2" src="'+WST.conf.ROOT+'/wstmart/admin/view/img/order_source_5.png"> ';
...
//上传头像
  WST.upload({
      ...
      callback:function(f){
        ...
        //将上传的图片路径赋给全局变量
        $('#userPhoto').val(json.thumb);
        $('#preview').html('<img src="'+json.thumb+'"  height="30" />');
        }else{
          WST.msg(json.msg,{icon:2});
        }
    },
