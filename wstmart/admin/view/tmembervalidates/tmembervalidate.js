...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'身份证照片', name:'id_card_positive', width: 20, renderer: function(val,item,rowIndex){
    ...
    mmg = $('.mmg').mmGrid({height: h-182,indexCol: true,indexColWidth:50, cols: cols,method:'POST',nowrap:true,
        url: WST.U('admin/tmembervalidates/pageQuery'), fullWidthRows: true, autoLoad: false,remoteSort: true,sortName:'createTime',sortStatus:'desc',
        remoteSort:true ,
        sortName: 'validateId',
        sortStatus: 'desc',
        plugins: [
...
function toEdit(id){
	location.href=WST.U('admin/tmembervalidates/toHandle','id='+id+'&p='+WST_CURR_PAGE);
}
function toView(id){
	location.href=WST.U('admin/tmembervalidates/toView','id='+id+'&p='+WST_CURR_PAGE);
}
function loadGrid(p){
    p=(p<=1)?1:p;
...
function save(p){
	...
	if(WST.confirm({content:'您确定该提现认证'+((params.status==2)?'成功':'失败')+'吗？',yes:function(){
	    ...
	    $.post(WST.U('admin/tmembervalidates/handle'),params,function(data,textStatus){
	    	layer.close(loading);
	    	var json = WST.toAdminJson(data);
	    	if(json.status=='1'){
	    		WST.msg("操作成功",{icon:1});
	    		location.href=WST.U('admin/tmembervalidates/index','p='+p);
	    	}else{
