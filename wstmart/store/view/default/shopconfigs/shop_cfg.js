...
$('#shopCfg').isValid(function(v){
    if(v){
            ...
            $.post(WST.U('store/shopconfigs/editShopCfg'),params,function(data,textStatus){
              layer.close(loading);
...
$(function(){
  ...
  WST.upload({
      ...
      callback:function(f){
        ...
        if(json.status==1){
          ...
          var shopbanner = json.thumb; //保存到数据库的路径
          $('#shopBanner').val(shopbanner);
          $('#shopBannerPreview').parent().show();
          $('.del-banner').show();
          $('#shopBannerPreview').attr('src',json.thumb);
        }else{
            WST.msg(json.msg,{icon:2});
        }
    },
  ...
  WST.upload({
      ...
      callback:function(f){
        ...
        if(json.status==1){
          $('#uploadMsg').empty().hide();
          var shopMoveBanner = json.thumb; //保存到数据库的路径
          $('#shopMoveBanner').val(shopMoveBanner);
          $('#shopMoveBannerPreview').parent().show();
          $('.del-banner').show();
          $('#shopMoveBannerPreview').attr('src',json.thumb);
        }else{
            WST.msg(json.msg,{icon:2});
        }
    },
  ...
  WST.upload({
      ...
      callback:function(f){
        ...
        if(json.status==1){
          ...
          $('.del-banner').show();
          $('#shopStreetImgPreview').attr('src',json.thumb);
        }else{
            WST.msg(json.msg,{icon:2});
        }
    },
...
/********** 轮播广告图片上传 **********/
var uploader = batchUpload({uploadPicker:'#batchUpload',uploadServer:WST.U('store/index/uploadPic'),formData:{dir:'shopconfigs'},uploadSuccess:function(file,response){
