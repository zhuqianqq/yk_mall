...
class Shops extends CShops{
    ...
    public function saveStep($data = []){
        ...
        //判断是否存在入驻申请
        $shops = $this->alias('s')->join('__SHOP_USERS__ sur','s.shopId=sur.shopId','left')->field('s.*')
            ->where(['sur.userId'=>$userId])->find();

        if(!empty($shops)){
            return WSTReturn('请勿重复申请入驻');
        }
        ...
        if($shops['applyStatus'] == 1){
            return WSTReturn('您的入驻申请正在审核，请勿重复提交');
        }
        ...
        $goodsCats = [];
        foreach($data as $k => $v){
            $field = Db::name('shop_bases')->where(['fieldName'=>$k,'dataFlag'=>1])
                ->field('fieldName,fieldType,fieldAttr,isShopsTable,dateRelevance,isShow,isRequire')->find();
            ...
            if($field['isShopsTable']==1){
                ...
                if($field['fieldType'] == 'other' && $field['fieldAttr'] == 'area'){
                    ...
                    if(!empty($areaIds)){
                        $shopsData[$k] = implode('_',$areaIds)."_";
                    }
                    if($field['fieldName'] == 'areaIdPath'){
                        $shopsData['areaId'] = $areaId;
                    }
                    if($field['fieldName'] == 'bankAreaIdPath'){
                        $shopsData['bankAreaId'] = $areaId;
                    }
                }
        ...
        try{
            ...
            if($flows[count($flows)-1]['flowId'] == $shopFlows['nextStep']['flowId']){
                $shopsData['applyStatus'] = 1; //是最后一个环节，状态改为审核中
