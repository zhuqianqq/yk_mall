...
function WSTUploadPic($fromType=0){
    ...
    if($info){
		...
		$info=null;
        //{"status":1,"savePath":"upload\/shopconfigs\/2020-03\/","name":"5e707d3b73a43.jpg","thumb":"5e707d3b73a43.jpg"}
		$rdata = [
		    'status' => 1,
            'savePath' => $filePath,
            'name' => $imgSrc,
 ...
 * 切换图片
 * @param $imgurl 图片路径
 * @param $imgType 图片类型    1:H5移动版轮播图    2:H5移动版缩略图
 * @param $default 图片默认    mallLogo:商城Logo goodsLogo:商品默认 shopLogo:店铺默认 userLogo:会员默认
 * 图片规则  

 * H5移动版轮播图 :http://examples-1251000004.picsh.myqcloud.com/sample.jpeg?imageView2/1/w/375/h/375/
 * H5移动版缩略图 :http://examples-1251000004.picsh.myqcloud.com/sample.jpeg?imageView2/1/w/85/h/85/
 */
function WSTImgH5($imgurl,$imgType = 0,$default = ''){
	
	$img = '';
	switch ($imgType){
		case 0:$img =  $imgurl;break;
		case 1:$img =  $imgurl . '?imageView2/1/w/375/h/375/';break;
		case 2:$img =  $imgurl . '?imageView2/1/w/85/h/85/';break;
	}
	return $img;
}
...
function WSTOrderNo(){
    ...
    $num = Cache::inc($redisKey);// redis读取，替换一下
    if (!$num) {
        $sn = date('Ymd') . '0001';
    } else {
...
function WSTCreateQrcode($qr_url,$logo_url="",$dir='images',$time=3600,$margin=0){
        ...
        require \think\facade\Env::get('root_path') . 'extend/qrcode/phpqrcode.php';
 ...
 * @param $imgUrl
 */
function buildImgUrl($imgUrl){
    if(empty($imgUrl)){
        return '';
    }else if(substr($imgUrl,0,4) == "http"){
        return $imgUrl;
    }
    return WSTConf('CONF.resourcePath') . "/" .ltrim($imgUrl,"/");
}